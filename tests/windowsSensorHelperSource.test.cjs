...
test('Windows helper regeneration is explicit while normal release builds verify the prebuilt asset', () => {
  ...
  assert.match(buildScript, /vendor\/openhardwaremonitor\/OpenHardwareMonitor\.exe/)
  assert.match(buildScript, /vendor\/openhardwaremonitor\/HWInfoXSensorHelper\.exe/)
  ...
  assert.match(buildScript, /`\/reference:\$\{backendAssembly\}`/)
