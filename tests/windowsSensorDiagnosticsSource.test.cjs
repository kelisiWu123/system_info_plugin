...
test('Windows sensor diagnostics classify empty, missing CPU, filter mismatch, and missing temperature states', () => {
  ...
  assert.match(service, /(?:async\s+)?function getWindowsProcessId\(processName\)/)
  assert.match(service, /const diagnosticHelperRunning = Boolean\(/)
  assert.match(service, /status\?\.helperRunning && \(processPresent \|\| snapshot\?\.received\)/)
  assert.match(service, /processId: helperStatus\?\.processId \?\? fallbackProcessId \?\? null/)
  ...
  assert.match(service, /allVoltageSamples/)
  assert.match(service, /vcoreCandidateSamples/)
  assert.match(service, /function isExplicitCpuVcoreSensor/)
  assert.match(service, /function isLikelySuperIoCpuVcoreSensor/)
  assert.match(service, /function isCpuVcoreSensor/)
  assert.match(service, /function isCpuVidSensor/)
  assert.match(service, /const vcoreSensors = usableSensors\.filter\(isCpuVcoreSensor\)/)
  assert.match(service, /const vidSensors = usableSensors\.filter\(isCpuVidSensor\)/)
  assert.match(service, /const measurement = vcoreSensors\.length \? 'vcore' : 'vid'/)
...
test('Processor shows and copies Windows helper diagnostics when enhanced CPU telemetry is still missing', () => {
  ...
  assert.match(processor, /cpuVoltageMetricLabel/)
  assert.match(processor, /核心 VID/)
  assert.match(processor, /vcoreCandidateSamples/)
  assert.match(processor, /allVoltageSamples/)
  assert.match(processor, /\[Raw sensor samples\]/)
  assert.match(processor, /overflow-x: hidden/)
  assert.match(processor, /overflow-wrap: anywhere/)
  assert.match(processor, /max-width: 42%/)
