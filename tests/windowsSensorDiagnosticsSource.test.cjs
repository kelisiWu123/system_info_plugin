...
test('Windows sensor diagnostics classify empty, missing CPU, filter mismatch, and missing temperature states', () => {
  ...
  assert.match(service, /cpuVoltageUsableCount/)
  assert.match(service, /cpuVoltageSamples/)
  assert.match(service, /usableSensors = sensors\.filter/)
  assert.match(service, /sensor\.value > 0/)
