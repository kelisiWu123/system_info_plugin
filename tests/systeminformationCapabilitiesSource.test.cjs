...
test('storage page consumes fs and disk IO telemetry from systeminformation', () => {
  ...
  assert.match(service, /const WINDOWS_STORAGE_IO_COUNTERS = \[/)
  assert.match(service, /async function readWindowsStorageIo\(\)/)
  assert.match(service, /typeperf\.exe/)
  assert.match(service, /parseWindowsTypeperfValues\(stdout\)/)
