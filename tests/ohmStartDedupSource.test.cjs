...
test('manual and automatic OpenHardwareMonitor starts share one deduplicated path', () => {
  const source = readProjectFile('utools/services/system.js')
  const bundledStartCalls = source.match(/startBundledOpenHardwareMonitor\(\)/g) || []

  assert.equal(bundledStartCalls.length, 2, 'expected one function declaration and one shared start invocation')
  ...
  assert.match(source, /OHM_START_IN_PROGRESS/)
})

test('OpenHardwareMonitor readiness detects an already-running local service before starting again', () => {
  const source = readProjectFile('utools/services/system.js')
  ...
  assert.match(source, /fetch\(`http:\/\/127\.0\.0\.1:\$\{port\}\/data\.json`/)
  assert.match(source, /if \(await isOpenHardwareMonitorRunning\(settings\)\)[\s\S]*markOpenHardwareMonitorStartAttempt\(\)[\s\S]*startBundledOpenHardwareMonitor\(\)/)
})
